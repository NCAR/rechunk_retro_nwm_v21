...
file_chunked = output_path / "rtout.zarr"
file_chunked_0 = file_chunked
...
def main():
    ...
    with open(file_lock) as f:
        lock_info = f.readlines()

    lock_d = {ll.split(': ')[0]: ll.split(': ')[1].rstrip()
              for ll in lock_info}

    start_date = lock_d['start_date']
    end_date  =  lock_d['end_date']
    file_chunked = pathlib.Path(
        str(file_chunked).replace(".zarr", "_fix.zarr"))
    if file_chunked.exists():
        raise FileExistsError(f'file_chunked exists:{file_chunked}')
    ...
    files = [
        pathlib.Path(
            f"{input_dir}/"
            f'{date.strftime("%Y")}/'
            f'{date.strftime("%Y%m%d%H%M")}.RTOUT_DOMAIN1.comp'
        )
        for date in dates
    ]

    n_chunks_job_actual = ceil(len(files) / time_chunk_size)
    assert n_chunks_job_actual == 1  # fix
    ...
    ds_full = xr.open_zarr(str(file_chunked_0), consolidated=True)
    ds_chunk = xr.open_zarr(str(file_chunked), consolidated=True)

    # get index of first new datapoint
    print(ds_full.time.size)
    start_ind, = np.nonzero(np.isin(ds_chunk.time, ds_full.time))
    #print(start_ix)
    # region of new data
    region_new = slice(start_ind[0], ds_chunk.time.size)
    #print(region_new)

    # get updated dataset size and create slice
    region_update = slice(
        ds_full.time.size - int(ds_chunk.time.size),
        ds_full.time.size)
    print(region_update)
    print(len(ds_chunk['time'].values))
    #print(ds_ondisk.time.size - int(ds_B.time.size))

    # write new data to zarr (all the coordinates except time needs to be dropped)
    ds_chunk = ds_chunk.drop(['x', 'y', 'crs'])
    _ = ds_chunk.to_zarr(file_chunked_0, region={"time": region_update})
    _ = ds_chunk.close()

    # produces
    ds = xr.open_zarr(file_chunked_0)
    print(ds)
    _ = ds.close()

    # unlink the fix.zarr file and the lock file?
    # _ = file_lock.unlink()
